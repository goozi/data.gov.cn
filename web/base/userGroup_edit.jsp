...
%>
  <tr class="odd"> 
      <td width="9%"  align="center"><img src='../images/delete.gif' style='cursor:hand' onclick='delete<%=type%>(this)'> 
        <input type="hidden" name="departmentIds" value="<%=baseUserGroup.getDepartmentId()%>">
        <input type="hidden" name="groupIds" value="<%=baseUserGroup.getGroupId()%>">
		<input type="hidden" name="sequs" value="<%=baseUserGroup.getSequ()%>">
        <input type="hidden" name="types" value="<%=baseUserGroup.getType()%>">
		<input type="hidden" name="ids" value="<%=baseUserGroup.getId()%>"></td>
...
%>
  <tr class="odd"> 
      <td width="9%"  align="center"><img src="../images/edit.gif" style='cursor:hand' onclick="doSelect<%=type%>()"> 
        ...
        <input type="hidden" name="types" value="0"><input type="hidden" name="ids" value="-1"></td>
      <td >&nbsp; </td>
      <td >&nbsp; </td>
      <td >&nbsp; </td>
  </tr>
...
function addRow<%=type%>(depName,depId,groupName,groupId,sequ)
{
	...
	sHTML+="<input type='hidden' name='ids' value='0'>";
